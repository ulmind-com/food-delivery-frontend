...
const CustomerChatDrawer = ({ isOpen, onClose }: CustomerChatDrawerProps) => {
    ...
    const handleSend = async () => {
        ...
        setText("");
        try {
            const res = await chatApi.sendMessage(optimisticMsg.text);
            ...
            setChat((prev) => {
                if (!prev) return prev;
                return { ...prev, messages: prev.messages.filter((m) => m._id !== optimisticMsg._id) };
            });
        } finally {
            setSending(false);
        }
            ...
            {isOpen && (
                <>
                    ...
                    >
                        ...
                        {/* Closed banner */}
                        {closed && (
                            <div className="border-t border-border bg-red-50 dark:bg-red-900/20 px-4 py-3 text-center text-xs font-medium text-red-600 dark:text-red-400">
